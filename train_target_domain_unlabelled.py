...
task_placeholder_labels = np.zeros((target_train_unlabelled_count,6))
mmd_placeholder_labels = np.zeros((target_train_unlabelled_count,256))
test_placeholder_labels = np.zeros((test_datapoints,6))
target_model.compile(loss={'predictions': empty,
                           'feature_layer': mmd2_rbf_X_quad(source_feature_layer)},
                     optimizer=adam)
target_model.fit(target_train_unlabelled, [task_placeholder_labels, mmd_placeholder_labels],
                 batch_size=batch_size,
                 epochs=epochs,
                 validation_data=(target_test,
...
target_model_path = "models/a6_target_10pUnlabelled.h5"
